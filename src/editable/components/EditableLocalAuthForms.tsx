...
const inputClass = 'h-[3.35rem] rounded-2xl border border-black/10 bg-white px-4 text-base font-bold text-[var(--slot4-page-text)] shadow-sm outline-none transition placeholder:text-black/35 focus:border-[#792ca2] focus:ring-4 focus:ring-[#792ca2]/10'
const buttonClass = 'inline-flex h-[3.35rem] items-center justify-center rounded-full bg-[var(--slot4-dark-bg)] px-6 text-xs font-black uppercase tracking-[0.18em] text-white shadow-[0_16px_35px_rgba(7,11,34,.18)] transition hover:-translate-y-0.5 hover:bg-[#792ca2] disabled:opacity-60'
...
export function EditableLocalLoginForm() {
  ...
  return (
    <form className="mt-6 grid gap-4" onSubmit={submit}>
      ...
      {message ? <p className={`rounded-2xl border px-4 py-3 text-sm font-bold ${status === 'success' ? 'border-emerald-200 bg-emerald-50 text-emerald-800' : 'border-red-200 bg-red-50 text-red-700'}`}>{message}</p> : null}
